...
def build_guest_dat(uid, password):
    return {"guest_account_info": {"com.garena.msdk.guest_password": password, "com.garena.msdk.guest_uid": uid}}
...
def webhook():
    try:
        ...
        if 'document' in message:
            ...
            if not accounts:
                send_message(chat_id, "❌ Tidak ada akun valid.")
                return "OK", 200
            
            send_message(chat_id, f"✅ Ditemukan {len(accounts)} akun. Mengirim file...")
            
            for acc in accounts:
                data = build_guest_dat(acc["uid"], acc["password"])
                json_str = json.dumps(data, separators=(',', ':'), ensure_ascii=False)
                send_document(chat_id, json_str.encode('utf-8'), f"guest100067.dat")
            
            return "OK", 200
        
        elif 'text' in message:
            if message['text'] == '/start':
                send_message(chat_id, "🔥 **Guest Converter Bot** 🔥\n\nKirim file `accs.json` (format `UID:PASSWORD`).")
            else:
                send_message(chat_id, "📤 Kirim file `accs.json`.")
